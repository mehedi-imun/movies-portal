...
const Cart = ({ onClose }: ICart) => {
  ...
  const handleRemove = (id: number) => {
    const nextData = cartData.filter((item: any) => item.id !== id);
      ...
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[420px] sm:max-w-[600px] lg:max-w-[790px] p-4 max-h-[90vh] overflow-auto">
        <div className="bg-white shadow-md dark:bg-[#12141D] rounded-2xl overflow-hidden p-5 md:p-9">
          ...
          <div className="space-y-8 lg:space-y-12 max-h-[450px] overflow-auto mb-10 lg:mb-14">
            {cartData.length == 0 ? (
              <span className="text-primary text-3xl"> The card is empty</span>
            ) : (
              cartData.map((items: any) => (
                <div key={items.id} className="grid grid-cols-[1fr_auto] gap-4">
                  <div className="flex items-center gap-4">
                    <img
                      className="rounded overflow-hidden size-24"
                      src={getImageUrl(items.cover)}
                      alt=""
                    />
                    <div>
